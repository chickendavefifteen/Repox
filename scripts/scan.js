...
async function scanNativeBalances() {
  ...
  for (const [chainName, chainConfig] of Object.entries(protocols.chains)) {
    try {
      ...
      balances[chainName] = { amount: formatted, usdValue: formatted * price, currency: chainConfig.nativeCurrency };
      if (formatted > 0) {
...
async function main() {
  ...
  const airdropResults = await Promise.allSettled(airdrops.map(a => scanAirdrop(a)));

  log('Scanning protocol reward contracts...');
  const rewardResults = await Promise.allSettled(protocols.rewardContracts.map(c => scanProtocolReward(c)));
